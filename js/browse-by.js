...
var BrowseByController = (function () {
   ...
   function getCards() {
      var currentVersion = parseInt(
         elements.contentGrid.getAttribute("data-version") || "0"
      );
      if (!cachedCards || currentVersion !== cacheVersion) {
         cachedCards = Array.from(
            elements.contentGrid.querySelectorAll(".content-card")
         );
   ...
   function init() {
         ...
         navButtons.forEach(function (btn) {
            btn.addEventListener("keydown", function (e) {
               if (e.keyCode === KeyCodes.DOWN) {
                  ...
                  e.stopPropagation();
                  console.log(
                     "[BrowseBy] DOWN from navbar, returning to controls"
                  );
      ...
      initializeJellyseerr()
         ...
         .then(function () {
            ...
            var savedPosition = sessionStorage.getItem(
               "browse_by_scroll_position"
            );
            if (savedPosition) {
               try {
                  ...
                  if (
                     position.browseType === browseType &&
                     position.browseId === browseId
                  ) {
                     setTimeout(function () {
   ...
   function getElements() {
      ...
      elements.activeFiltersDisplay = document.getElementById(
      ...
      elements.globalBackdropImage = document.getElementById(
         "globalBackdropImage"
      );
   ...
   function parseUrlParams() {
      ...
      if (browseType === "genre") {
         ...
         elements.headerLogo.alt =
            browseName || (browseType === "studio" ? "Studio" : "Network");
   ...
   function initializeJellyseerr() {
      ...
      if (typeof tizen !== "undefined") {
         console.log(
            "[BrowseBy] Tizen detected - checking for API key auth..."
         );
         ...
         if (settingsStr) {
            try {
               ...
               console.log(
                  "[BrowseBy] Settings - jellyseerrApiKey:",
                  settings.jellyseerrApiKey ? "SET" : "NOT SET"
               );
               console.log(
                  "[BrowseBy] Settings - jellyseerrUrl:",
         ...
         console.log(
            "[BrowseBy] Tizen with API key - initializing directly..."
         );
      ...
      return JellyseerrAPI.initializeFromPreferences().then(function (success) {
         ...
         console.log(
            "[BrowseBy] No stored session, initializing with server URL"
         );
         var settingsStr = storage.getUserPreference("jellyfin_settings", null);
         if (!settingsStr) {
         ...
         return JellyseerrAPI.initialize(
            ...
            .then(function (loginSuccess) {
               if (loginSuccess) {
                  ...
                  console.log(
                     "[BrowseBy] Auto-login failed, API initialized without auth"
                  );
                  return Promise.resolve();
               }
            });
   ...
   function setupEventListeners() {
      ...
      var filterOptions =
         elements.filterModal.querySelectorAll(".modal-option");
   ...
   function loadContent() {
      ...
      apiMethod
         .call(JellyseerrAPI, discoverOptions)
         .then(function (response) {
            ...
            console.log(
               "[BrowseBy] Before filter, items count:",
               items.length,
               "Filter:",
               currentFilter
            );
         ...
         .catch(function (error) {
            ...
            if (error.message && error.message.includes("Session expired")) {
               JellyseerrAPI.handleSessionExpiration(function () {
                  loadContent();
                  return Promise.resolve();
               }, "BrowseBy").catch(function (retryError) {
                  console.error(
                     "[BrowseBy] Re-initialization failed:",
                     retryError
                  );
   ...
   function loadMoreContent() {
      ...
      apiMethod
         ...
         .then(function (response) {
            ...
            console.log(
               "[BrowseBy] Filtered new items count:",
               newItems.length
            );
   ...
   function createCard(item, index) {
      ...
      if (item.posterPath) {
         ...
         placeholderImg.src = "assets/icons/movie.png";
         placeholderImg.alt = "";
         placeholderImg.className = "placeholder-icon";
      ...
      if (item.voteAverage) {
         ...
         starImg.alt = "";
         starImg.className = "rating-star-icon";
         rating.appendChild(starImg);
         rating.appendChild(
            document.createTextNode(" " + item.voteAverage.toFixed(1))
         );
      ...
      card.addEventListener("focus", function () {
         ...
         if (item.backdropPath && elements.globalBackdropImage) {
            var backdropUrl = ImageHelper.getTMDBImageUrl(
               item.backdropPath,
               "original"
            );
   ...
   function handleKeyDown(event) {
      ...
      console.log(
         "[BrowseBy] Key pressed:",
         keyCode,
         "Modal:",
         focusManager.inModal,
         "Controls:",
         focusManager.inControls,
         "Grid:",
         focusManager.inGrid,
         "GridIndex:",
         focusManager.currentGridIndex
      );
      ...
      if (keyCode === KeyCodes.UP) {
         ...
         if (focusManager.inControls) {
            ...
            console.log(
               "[BrowseBy] Grid calculations - Width:",
               gridWidth,
               "Cols:",
               cols,
               "Cards:",
            ...
            if (currentIndex < cols) {
               ...
               console.log(
                  "[BrowseBy] Moving up in grid from",
                  currentIndex,
                  "to",
                  newIndex
               );
         ...
         if (focusManager.inControls) {
            ...
            if (newIndex !== currentIndex) {
               console.log(
                  "[BrowseBy] Moving down in grid from",
                  currentIndex,
                  "to",
                  newIndex
               );
               ...
               if (rowsFromEnd <= 2 && currentPage < totalPages && !isLoading) {
                  console.log(
                     "[BrowseBy] Near bottom (rows from end:",
                     rowsFromEnd,
                     "), loading next page:",
                     currentPage + 1,
                     "of",
               ...
               console.log(
                  "[BrowseBy] At bottom, loading next page:",
                  currentPage + 1,
                  "of",
                  totalPages
               );
         ...
         if (focusManager.inControls) {
            ...
            if (currentIndex > 0) {
               console.log(
                  "[BrowseBy] Moving left in grid from",
                  currentIndex,
                  "to",
                  currentIndex - 1
               );
         ...
         if (focusManager.inControls) {
            ...
            if (currentIndex < cards.length - 1) {
               console.log(
                  "[BrowseBy] Moving right in grid from",
                  currentIndex,
                  "to",
                  currentIndex + 1
               );
   ...
   function handleModalKeyDown(event) {
      ...
      var options = Array.from(
         modal.querySelectorAll(".modal-option, .modal-close")
      );
   ...
   function openSortModal() {
      ...
      var options = Array.from(
         elements.sortModal.querySelectorAll(".modal-option")
      );
      if (options.length > 0) {
         options[0].focus();
      }
