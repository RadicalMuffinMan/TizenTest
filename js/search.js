var SearchController = (function () {
   ...
   function initializeJellyseerr() {
      ...
      if (typeof tizen !== "undefined") {
         ...
         if (settingsStr) {
            try {
               ...
               console.log(
                  "[Search] Settings - jellyseerrApiKey:",
                  settings.jellyseerrApiKey ? "SET" : "NOT SET"
               );
               console.log(
                  "[Search] Settings - jellyseerrUrl:",
      ...
      return JellyseerrAPI.initializeFromPreferences().then(function (success) {
         ...
         console.log(
            "[Search] initializeFromPreferences returned false, trying direct initialization"
         );
         var settingsStr = storage.getUserPreference("jellyfin_settings", null);
         if (!settingsStr) {
            jellyseerrEnabled = false;
         ...
         try {
            ...
            return JellyseerrAPI.initialize(
               ...
               .then(function () {
                  ...
                  return JellyseerrAPI.attemptAutoLogin().then(function (
                     loginSuccess
                  ) {
               ...
               .catch(function (error) {
                  console.error(
                     "[Search] Direct initialization failed:",
                     error
                  );
                  jellyseerrEnabled = false;
                  return false;
   ...
   function handleGlobalKeyDown(evt) {
      ...
      switch (evt.keyCode) {
         ...
         case KeyCodes.RIGHT:
            ...
            if (
               currentRowData &&
               focusManager.currentItem < currentRowData.length - 1
            ) {
               focusManager.currentItem++;
               updateFocus();
            }
      ...
      ajax.request(url, {
         ...
         success: function (response) {
            ...
            if (jellyseerrEnabled) {
               ...
               JellyseerrAPI.search(query)
                  .then(function (jellyseerrResponse) {
                     console.log(
                     ...
                     console.log(
                        "Added",
                        currentResults.jellyseerr.length,
                        "Jellyseerr results (actors excluded)"
                     );
   ...
   function displayResults() {
      ...
      if (currentResults.episodes.length > 0) {
         ...
         renderResultCards(
            currentResults.episodes,
            elements.episodesList,
            "episode"
         );
      } else {
      ...
      if (currentResults.jellyseerr && currentResults.jellyseerr.length > 0) {
         ...
         renderResultCards(
            currentResults.jellyseerr,
            elements.jellyseerrList,
            "jellyseerr"
         );
      } else {
         ...
         if (imageTag) {
            if (type === "episode") {
               imageUrl =
                  ...
                  "/Items/" +
                  item.Id +
                  "/Images/Primary?fillWidth=500&quality=90";
            } else {
               imageUrl =
                  auth.serverAddress +
                  "/Items/" +
   ...
   function getCurrentRowData() {
      ...
      if (
         ...
         focusManager.currentRow >= visibleRows.length
      ) {
         return null;
      }
   ...
   function getContainerForType(type) {
      switch (type) {
         case "movies":
            return elements.moviesList;
         case "shows":
            return elements.showsList;
         case "episodes":
            return elements.episodesList;
         case "people":
            return elements.castList;
         case "jellyseerr":
            return elements.jellyseerrList;
         default:
            return null;
      }
   }
   ...
   function selectCurrentItem() {
      ...
      if (
         !currentRowData ||
         focusManager.currentItem >= currentRowData.length
      ) {
         return;
      }
   ...
   function showNoResults(query) {
      ...
      if (elements.noResultsQuery) {
         elements.noResultsQuery.textContent =
