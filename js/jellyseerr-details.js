...
var JellyseerrDetailsController = (function () {
   ...
   function initializeJellyseerr() {
      ...
      if (typeof tizen !== "undefined") {
         console.log(
            "[Jellyseerr Details] Tizen detected - checking for API key auth..."
         );
         ...
         if (settingsStr) {
            try {
               ...
               console.log(
                  "[Jellyseerr Details] Settings - jellyseerrApiKey:",
                  settings.jellyseerrApiKey ? "SET" : "NOT SET"
               );
               console.log(
                  "[Jellyseerr Details] Settings - jellyseerrUrl:",
                  settings.jellyseerrUrl || "NOT SET"
               );
               hasApiKey =
                  settings.jellyseerrApiKey &&
                  settings.jellyseerrApiKey.length > 0;
               hasUrl =
                  settings.jellyseerrUrl && settings.jellyseerrUrl.length > 0;
            } catch (e) {
         ...
         if (!hasApiKey || !hasUrl) {
            console.log(
               "[Jellyseerr Details] No Jellyseerr configuration found"
            );
         ...
         console.log(
            "[Jellyseerr Details] Tizen with API key - initializing directly..."
         );
         ...
         console.log(
            "[Jellyseerr Details] API Key length:",
            jellyseerrApiKey ? jellyseerrApiKey.length : 0
         );
         ...
         // Initialize with URL
         return JellyseerrAPI.initialize(
            jellyseerrUrl,
            jellyseerrApiKey,
            userId
         )
            .then(function () {
               console.log(
                  "[Jellyseerr Details] JellyseerrAPI.initialize() succeeded"
               );
               console.log(
                  "[Jellyseerr Details] isInitialized:",
                  JellyseerrAPI.isInitialized()
               );
               return true;
            })
            .catch(function (error) {
               console.error(
                  "[Jellyseerr Details] Error during initialization:",
                  error
               );
               return false;
            });
      ...
      return JellyseerrAPI.initializeFromPreferences().then(function (success) {
         if (success) {
            console.log(
               "[Jellyseerr Details] initializeFromPreferences succeeded"
            );
            return success;
         }
         ...
         console.log(
            "[Jellyseerr Details] initializeFromPreferences returned false, trying direct initialization"
         );
         var settingsStr = storage.getUserPreference("jellyfin_settings", null);
   ...
   function loadMediaDetails() {
      console.log(
         "[Jellyseerr Details] Loading details for",
         mediaType,
         mediaId
      );
      ...
      detailsPromise
         ...
         .catch(function (error) {
            console.error(
               "[Jellyseerr Details] Failed to load details:",
               error
            );
   ...
   function renderMediaDetails() {
      ...
      if (mediaData.backdropPath) {
         elements.backdropImage.src = ImageHelper.getTMDBImageUrl(
            mediaData.backdropPath,
            "original"
         );
      }
      ...
      if (mediaData.posterPath) {
         elements.posterImage.src = ImageHelper.getTMDBImageUrl(
            mediaData.posterPath,
            "w500"
         );
      }
      ...
      elements.mediaTitle.textContent =
         mediaData.title || mediaData.name || "Unknown";
      ...
      if (mediaData.voteAverage) {
         elements.mediaRating.innerHTML =
            '<img src="assets/icons/star.png" alt="" class="rating-star-icon"> ' +
            mediaData.voteAverage.toFixed(1);
      ...
      if (mediaData.genres && mediaData.genres.length > 0) {
         var genreNames = mediaData.genres
            .map(function (g) {
               return g.name;
            })
            .join(", ");
   ...
   function updateRequestButtons() {
      ...
      var request4kLabel =
         elements.request4kBtnWrapper.querySelector(".btn-label");
      if (status4k === 2) {
   ...
   function updateSimilarTitle() {
      if (elements.similarTitle) {
         elements.similarTitle.textContent =
            mediaType === "tv" ? "Similar Series" : "Similar Titles";
      }
   }
   ...
   function loadRecommendations(page) {
      ...
      if (recommendationsPagination.isLoading) {
         console.log(
            "[Jellyseerr Details] Already loading recommendations, skipping"
         );
         return Promise.resolve();
      }

      console.log(
         "[Jellyseerr Details] Loading recommendations page:",
         page,
         "mediaType:",
         mediaType,
         "mediaId:",
      ...
      return recommendationsPromise
         .then(function (response) {
            console.log(
               "[Jellyseerr Details] Recommendations page",
               page,
               "loaded successfully:",
               response
            );
         ...
         .catch(function (error) {
            console.error(
               "[Jellyseerr Details] Failed to load recommendations:",
               error
            );
            ...
            if (
               error &&
               error.message &&
               error.message.indexOf("Session expired") !== -1
            ) {
               console.log(
                  "[Jellyseerr Details] Session expired during recommendations load, attempting recovery"
               );
               ...
               JellyseerrAPI.handleSessionExpiration(function () {
                  console.log(
                     "[Jellyseerr Details] Session re-initialized, retrying recommendations page",
                     page
                  );
                  ...
                  .then(function () {
                     console.log(
                        "[Jellyseerr Details] Retry successful for recommendations page",
                        page
                     );
                  })
                  .catch(function (retryError) {
                     console.error(
                        "[Jellyseerr Details] Retry failed for recommendations page",
                        page,
                        retryError
                     );
   ...
   function createRecommendationCard(item) {
      ...
      card.addEventListener("click", function () {
         window.location.href =
            "jellyseerr-details.html?type=" + item.mediaType + "&id=" + item.id;
      });
   ...
   function loadSimilar(page) {
      ...
      return similarPromise
         ...
         .catch(function (error) {
            console.error(
               "[Jellyseerr Details] Failed to load similar content:",
               error
            );
            similarPagination.isLoading = false;
            ...
            if (
               ...
               console.log(
                  "[Jellyseerr Details] Session expired during similar load"
               );
               ...
               JellyseerrAPI.handleSessionExpiration(function () {
                  console.log(
                     "[Jellyseerr Details] Session re-initialized after similar error"
                  );
               }, "Similar Load");
            } else {
               // For other errors, stop pagination
   ...
   function navigateToKeyword(keywordId, keywordName) {
      var url =
         "browse-by.html?type=keyword&id=" +
         keywordId +
         "&name=" +
         encodeURIComponent(keywordName) +
         "&mediaType=" +
         mediaType;
   ...
   function selectAllSeasons() {
      var checkboxes = elements.seasonList.querySelectorAll(
         'input[type="checkbox"]'
      );
   ...
   function submitRequest(seasons, is4k) {
      ...
      JellyseerrAPI.createRequest(requestData)
         .then(function (response) {
            showSuccessMessage(
               (is4k ? "4K" : "HD") + " request submitted successfully!"
            );
   ...
   function navigateToPerson(personId, personName) {
      console.log(
         "[Jellyseerr Details] Navigating to person:",
         personId,
         personName
      );
      window.location.href =
         "jellyseerr-person.html?id=" +
         personId +
         "&name=" +
   ...
   function handleRequestError(error) {
      ...
      if (error.status === 403) {
         message =
            "You do not have permission to request content. Please contact your administrator to grant you request permissions in Jellyseerr.";
      } else if (error.status === 401) {
         message =
            "Authentication failed. Please check your Jellyseerr connection in settings.";
      } else if (error.status === 409) {
         message =
            "This content has already been requested or is currently available.";
      ...
      try {
         ...
         if (typeof tizen !== "undefined" && tizen.application) {
            tizen.application.launchAppControl(
               ...
               function (e) {
                  console.error(
                     "[JellyseerrDetails] YouTube launch failed:",
                     e
                  );
   ...
   function handlePlay() {
      ...
      var year = mediaData.releaseDate
         ? new Date(mediaData.releaseDate).getFullYear()
         : mediaData.firstAirDate
         ? new Date(mediaData.firstAirDate).getFullYear()
         : null;
      ...
      if (!auth) {
         window.location.href =
            "search.html?query=" + encodeURIComponent(searchTitle);
         return;
      }
   ...
   function navigateToSimilar(id, type) {
      window.location.href =
         "jellyseerr-details.html?type=" + type + "&id=" + id;
   }
   ...
   function getActionButtons() {
      return Array.from(document.querySelectorAll(".btn-action")).filter(
         function (btn) {
   ...
   function checkRecommendationsLoadMore(currentIndex, totalItems) {
      if (
         ...
         recommendationsPagination.isLoading
      ) {
         return;
      }
      ...
      if (itemsFromEnd <= 3) {
         console.log(
            "[Jellyseerr Details] Loading more recommendations, page:",
            recommendationsPagination.currentPage + 1
         );
   ...
   function checkSimilarLoadMore(currentIndex, totalItems) {
      ...
      if (itemsFromEnd <= 3) {
         console.log(
            "[Jellyseerr Details] Loading more similar content, page:",
            similarPagination.currentPage + 1
         );
      ...
      switch (evt.keyCode) {
         ...
         case KeyCodes.OK:
            ...
            if (buttons[focusManager.currentIndex]) {
               console.log(
                  "[Jellyseerr Details] ENTER pressed on button:",
                  buttons[focusManager.currentIndex].id
               );
      ...
      switch (evt.keyCode) {
         ...
         case KeyCodes.RIGHT:
            ...
            if (currentIndex < recommendationsCards.length - 1) {
               ...
               checkRecommendationsLoadMore(
                  currentIndex,
                  recommendationsCards.length
               );
            }
            break;
      ...
      switch (evt.keyCode) {
         ...
         case KeyCodes.DOWN:
            ...
            if (currentRowIndex < rows.length - 1) {
               ...
               // Try to maintain column position, or go to last item in row if shorter
               var targetDownColIndex = Math.min(
                  currentColIndex,
                  nextRow.length - 1
               );
   ...
   function handleModalKeyDown(evt) {
      ...
      var checkboxes = Array.from(
         elements.seasonList.querySelectorAll(".season-checkbox")
      );
      var actionButtons = [
         elements.confirmRequestBtn,
         elements.cancelRequestBtn,
      ];

      var allFocusable = modalButtons
         .concat(checkboxes)
